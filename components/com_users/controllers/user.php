...
class UsersControllerUser extends UsersController
{
	...
	public function login()
	{
		...
		{
			$data['return'] = 'index.php?option=com_content&view=article&id=71';
		}
		...
		else
		{
			...
			$data['error']='Error en usuario y contraseña';
			$app->setUserState('users.login.form.data', $data);

			$authenticate = JAuthentication::getInstance();
			$response = $authenticate->authenticate($credentials, $options);
			$response->status        = JAuthentication::STATUS_FAILURE;
			$response->error_message = JText::_('Usuario y/o Contraseña incorrectos');
			$app->redirect(JRoute::_(JPATH_AUTENTICARSE, true));
	...
	public function logout()
	{
		...
		if (!($error instanceof Exception))
		{
			...
			// Redirect the user.
			$app->redirect(JRoute::_(JPATH_AUTENTICARSE, false));
		}
		else
		{

			$app->redirect(JRoute::_(JPATH_ROOT, false));
		}
	}

...
class UsersModelProfile extends JModelForm
{
	...
	public function save($data)
	{
		...
		//$fileupload = JControllerLegacy::getInstance('Media', array('base_path' => JPATH_COMPONENT_ADMINISTRATOR));

		$persona = array();
		$persona['id_persona']= $user->id;
		$persona ['dni']= $user->dni;
		$persona ['nombre']= $user->name;
		$persona ['direccion']=$user->direccion;
		$persona ['correo']=$user->email;
		$persona ['telefono']=$user->telefono;
		$persona ['movil'] = $user->movil;
		$persona ['username']=$user->username;
		$persona ['password']=$user->password;
		//$persona ['foto']= $this->input->files->get('FileData','','name');
		$persona ['foto']=$_FILES["jform"]["name"];

//$controller= JControllerLegacy::getInstance('MediaFile', array('base_path' => JPATH_COMPONENT_ADMINISTRATOR));
//$controller->upload();

		$personamodel=JModelLegacy::getInstance('persona');
		$personamodel->bind($persona);

		foreach ($_FILES["jform"]["error"] as $key => $error) {
			if ($error == UPLOAD_ERR_OK) {
				$tmp_name = $_FILES["jform"]["tmp_name"][$key];
				$name = $user->username.'.jpg';
				//$_FILES["jform"]["name"][$key];
				move_uploaded_file($tmp_name, JPATH_IMG_UPLOAD.DIRECTORY_SEPARATOR.$name);
			}
